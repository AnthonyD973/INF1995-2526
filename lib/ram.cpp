...
/* Et par Anthony Dentinger                                                   */
/* Pour la librairie du projet de première année                              */
/*                                                                            */
/******************************************************************************/
...
/*                                                                            */
/*     Compilateur            AVR-GCC                                         */
/*     Microcontroleur        Atmel atmega324pa                               */
/*                                                                            */
/******************************************************************************/
...
# warning "F_CPU pas defini pour 'memory.cpp'"
# define F_CPU 8000000UL
#endif

uint8_t RAM::_mDeviceAddress = 0xA0;
const uint8_t RAM::_PAGE_SIZE = 128;
...
/******************************************************************************/
uint8_t RAM::chooseBank(const uint8_t bank)
{
   uint8_t temp = bank & 0x03;
   uint8_t rv = 255;
   if(bank == temp)
   {
      RAM::_mDeviceAddress = (0xA0 | ( bank << 1 ));
      rv = RAM::_mDeviceAddress;
   }
   return rv;
}
...
/*                                                                            */
/******************************************************************************/
uint8_t RAM::read(const uint16_t address, uint8_t *data)
{
  uint8_t rv = 0;
  ...
  for EVER
  {
    ...
    TWDR = _mDeviceAddress;    //controle - bit 0 a 0, ecriture
  ...
  //__________________ Transmission du code de controle ___________________
  TWDR =  _mDeviceAddress + 1;   // Controle - bit 0 a 1 lecture 
  TWCR = _BV(TWINT) | _BV(TWEN);        // R. a Z., interrupt. - Depart de transm.
  while ((TWCR & _BV(TWINT)) == 0)      // Attente de fin de transmission
    ;
...
uint8_t RAM::read(const uint16_t address, uint8_t *data, uint8_t length)
{
  uint8_t twcr;
...
/*                                                                            */
/******************************************************************************/
uint8_t RAM::write(const uint16_t address, const uint8_t data)
{
  //______________ Attente de la fin d'un cycle d'ecriture ______________
  for EVER
  {
...
uint8_t RAM::write(const uint16_t address, uint8_t *data,
                                const uint8_t length)
{
  uint8_t rv;
  uint16_t addrCopy = address;
  uint8_t lenCopy = length;
  do
  {
      rv = _writePage(addrCopy, data, lenCopy);
      addrCopy += rv;      // On pointe une nouvelle page
      lenCopy -= rv;     // On soustrait la partie ecrite precedemment
...
uint8_t RAM::_writePage(const uint16_t address, uint8_t *data,
                                   const uint8_t length)
{
  uint16_t endAddr;
  uint8_t rv = 0;
  ...
  if (address + length < (address | (_PAGE_SIZE - 1)))
    endAddr = address + length;
  else
    endAddr = (address | (_PAGE_SIZE - 1)) + 1;
  lenCopy = endAddr - address;
